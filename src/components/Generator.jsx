...
export default function Generator(props) {
  ...
  // function toggleModal() { // This function is not working as expected
  //   showModal = !showModal // It is because to handle anything interactive in React, we need to use stateful variables 
  // } // We can use the useState hook to create a state variable and a function to update it
  ...
  function updateMuscles(muscleGroup) {
    if (muscles.includes(muscleGroup)) {
      ...
      // console.log(muscles)  // Why this logs the previous state?
      // This is because the state update is asynchronous in React
      // To get the updated state, we can use the second argument of the useState hook
      // The second argument is a function that gets the previous state as an argument
      // We can use this function to get the updated state
      return
    }
    ...
    <SectionWrapper id='generate' header={'generate your workout'} title={['It\'s', 'Huge', 'o\'clock']}>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-4 gap-4'>
          {Object.keys(WORKOUTS).map((type, typeIndex) => {
            return (
              <button onClick={() => {
                setMuscles([]) // This is to reset the muscles when the poison is changed
                setPoison(type)
